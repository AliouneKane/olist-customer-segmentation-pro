...
def _segment_card(cluster_id: int, name: str, icon: str,
                  who: str, priority: str, action: str) -> None:
...
def render() -> None:
    ...
    tab_rfm, tab_exp, tab_sat, tab_pay = st.tabs(
        ["📦 RFM (comportement achat)", "🚚 Expérience livraison",
         "⭐ Satisfaction", "💳 Paiement & CLV"]
    )

    with tab_rfm:
        _metric_card(
            "Récence (Recency)", icon="📅",
    ...
    with tab_exp:
        _metric_card(
            "Délai de livraison (avg_delivery_delay)", icon="📬",
        ...
        _metric_card(
            "Ratio fret / panier (avg_freight_ratio)", icon="🚛",
    ...
    with tab_sat:
        _metric_card(
            "Note de satisfaction (avg_review_score)", icon="⭐",
    ...
    with tab_pay:
        _metric_card(
            "CLV Proxy (Customer Lifetime Value)", icon="💎",
        ...
        _metric_card(
            "Mode de paiement (payment_type_cc_flag)", icon="💳",
        ...
        _metric_card(
            "Nombre de versements (avg_installments)", icon="📋",
