"""Generate a PDF report with all key results from notebook 04_simulation."""
...
OUTPUT_PDF = PROJECT_ROOT / "reports" / "04_simulation_results.pdf"
...
def styled_fig(figsize=(11.69, 8.27)):
...
def title_page(pdf):
    ...
    fig.text(0.5, 0.62, "RAPPORT DE SIMULATION", ha="center", va="center",
             fontsize=32, fontweight="bold", color=C_TEXT, family="sans-serif")
    fig.text(0.5, 0.52, "Analyse de Stabilité Temporelle du Clustering",
             ha="center", va="center", fontsize=18, color=C_BLUE)
    fig.text(0.5, 0.42, "Notebook 04 — Olist Customer Segmentation",
             ha="center", va="center", fontsize=14, color=C_MUTED)
    fig.text(0.5, 0.30, f"Généré le {datetime.now().strftime('%d/%m/%Y à %H:%M')}",
...
def text_page(pdf, title, lines):
    ...
    for line in lines:
        ...
        if line.startswith("✅"):
            ...
            color = C_RED
        fig.text(0.08, y, line, ha="left", va="top",
                 fontsize=12, color=color, family="monospace", wrap=True,
...
def table_page(pdf, title, df, col_widths=None):
    fig, ax = plt.subplots(figsize=(11.69, 8.27), facecolor=C_BG)
    ax.set_facecolor(C_BG)
    ax.axis("off")
    fig.text(0.5, 0.95, title, ha="center", va="top",
             fontsize=20, fontweight="bold", color=C_BLUE)
    ...
    for (row, col), cell in tbl.get_celld().items():
        if row == 0:
            ...
            cell.set_facecolor(C_CARD if row % 2 == 0 else C_BG)
            cell.set_text_props(color=C_TEXT)
        cell.set_edgecolor(C_MUTED)
...
def main():
    print("=" * 60)
    print("  GÉNÉRATION DU RAPPORT PDF — Notebook 04 Simulation")
    print("=" * 60)
    ...
    assert model_files, "Aucun modèle trouvé."
    ...
    for name, df_w in windows_data.items():
        ...
        if len(X_w) < BEST_K * 10:
            print(f"  {name} ignoré (trop peu de données)")
            continue
    ...
    for t_prev, t_next in pairs:
        ...
        print(f"  ARI {t_prev}→{t_next} = {ari:.4f} ({len(common):,} communs)")
    ...
    print(f"  Bootstrap: mean={boot_mean:.4f} std={boot_std:.4f} CV={boot_cv:.4f}")
    ...
    with PdfPages(str(OUTPUT_PDF)) as pdf:
        ...
        # Page 2: Configuration summary
        config_lines = [
            f"Modèle        : {best_model_path.name}",
            f"Algorithme    : {meta.get('algorithm', 'KMeans')}",
            f"k optimal     : {BEST_K}",
            f"Features ({len(FINAL_FEATURES)})  : {', '.join(FINAL_FEATURES[:5])}...",
            f"Clients total : {len(df_labeled):,}",
            "",
            "── Fenêtres temporelles ──",
        ]
        for w, cutoff in WINDOWS.items():
            config_lines.append(f"  {w} ≤ {cutoff.date()} : {window_sizes[w]:>7,} clients")
        text_page(pdf, "Configuration de la Simulation", config_lines)

        # Page 3: Metrics per window (table)
        metrics_rows = []
        ...
        if metrics_rows:
            ...
            table_page(pdf, "Métriques de Clustering par Fenêtre", df_tbl,
                       col_widths=[0.12, 0.15, 0.18, 0.22, 0.25])

        # Page 4: ARI bar chart
        if ari_scores:
            fig, ax = styled_fig()
            ...
            ax.set_title("Adjusted Rand Index — Fenêtres Consécutives", fontsize=16, color=C_TEXT, pad=15)
            ax.legend(fontsize=11, facecolor=C_CARD, edgecolor=C_MUTED, labelcolor=C_TEXT)
        ...
        if silhouette_by_window:
            ...
            for w, v in zip(w_list, s_vals):
                ax.annotate(f"{v:.4f}", (w, v), textcoords="offset points",
                            xytext=(0, 12), ha="center", fontsize=12, color=C_TEXT)
            if s_vals:
            ...
            ax.set_ylabel("Score de Silhouette", fontsize=14)
            ax.set_title("Évolution du Score de Silhouette par Fenêtre",
                         fontsize=16, color=C_TEXT, pad=15)
            ax.legend(fontsize=11, facecolor=C_CARD, edgecolor=C_MUTED, labelcolor=C_TEXT)
            plt.tight_layout()
            pdf.savefig(fig, facecolor=fig.get_facecolor())
            plt.close(fig)

        # Page 6: Cluster profile T4
        if "T4" in profile_by_window:
            ...
            table_page(pdf, "Profil des Clusters — Fenêtre T4 (Dataset complet)", prof_display)

        # Page 7: Delta CLV T3→T4
        if delta_df is not None and len(delta_df) > 0:
            ...
            table_page(pdf, "Delta CLV Proxy — T3 → T4", dd)

        # Page 8: Bootstrap histogram
        if bootstrap_sil:
            fig, ax = styled_fig()
            ax.hist(bootstrap_sil, bins=20, color=C_BLUE, edgecolor="white", alpha=0.85)
            ax.axvline(boot_mean, color=C_RED, linewidth=2,
                       label=f"Moyenne = {boot_mean:.4f}")
            ...
            ax.set_xlabel("Score de Silhouette (bootstrap)", fontsize=14)
        ...
        # Page 9: Signals & Recommendation
        signal_lines = [
            "── SYNTHÈSE DES SIGNAUX DE STABILITÉ ──",
            "",
        ]
        for crit, val, status, rec in signals:
            signal_lines.append(f"{status}  {crit} = {val}  →  {rec}")
        ...
        signal_lines += [
            ...
            "━" * 50,
            "",
            f">>> RECOMMANDATION FINALE : Ré-entraînement {final_rec.upper()} <<<",
            "",
            "── Grille de décision ──",
            "",
            "  ARI > 0.7            →  ✅ Semestriel",
            "  ARI 0.5 – 0.7       →  ⚠️  Trimestriel",
            "  ARI < 0.5            →  🚨 Mensuel + alerting",
            "  Delta CLV > 20%      →  🚨 Ré-entraînement immédiat",
            "  CV bootstrap < 0.05  →  ✅ Modèle robuste",
        ]
        text_page(pdf, "Recommandation de Fréquence de Mise à Jour", signal_lines)

    print(f"\n✅ PDF généré avec succès : {OUTPUT_PDF}")
    print(f"   {OUTPUT_PDF.stat().st_size / 1024:.0f} Ko — 9 pages")
